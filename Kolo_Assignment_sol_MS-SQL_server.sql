...
/* Inserting 3 month of data for this assignment */
...
/* Montly Retention rate query for the given data which is scalable w.r.t to data addition */

with t as (
select a.*, b.total_count from

(select year(l_month.login_date) as Year_No, month(l_month.login_date) as month_no,
...
group by year(l_month.login_date), month(l_month.login_date)) as a
...
select Year_No, Month_No, (rep_cus*100/total_count) as Retenetion_Rate from t;

--------------------------------------------------------------------------------------------------------------------

/* Daily retention count of user as per the given data */
